// UI-related functionality for SplitInvoice
 ...
 class UI {
   static init() {
     ...
     if (bottomNav) {
       this.initFancyBottomNav(); // Initialize animations, history, settings buttons
        ...
        return; // Exit init early as createBottomNavIfMissing handles the rest
     }

     // Always try to set the active nav item based on the current page URL if nav exists
     this.setActiveNavItem(); // This should now work correctly on page load
   } // End of init() method
   ...
   static setActiveNavItem() {
       ...
       const navItems = bottomNav.querySelectorAll('.nav-item'); // Query within the nav
       if (!navItems.length) {
       ...
       // console.log(`setActiveNavItem: Current Page Filename = ${currentPageFilename}`); // Optional Debug

        console.log(`[Debug] setActiveNavItem: Current Page Filename = ${currentPageFilename}`); // Added for debugging
       navItems.forEach(item => {
           item.classList.remove('active'); // Ensure all are inactive first

           // Find the anchor tag, whether it's the item itself or a child
           ...
           if (link && link.tagName === 'A') {
               ...
               if (itemHrefFull && itemHrefFull !== '#') { // Ignore placeholder links
                   ...
                   const itemHrefFilename = hrefSegments.pop() || 'index.html';

                   // console.log(`setActiveNavItem: Processing ${itemHrefFull}, Extracted filename: ${itemHrefFilename}`); // Optional Debug
                    console.log(`[Debug] setActiveNavItem: Comparing with Item Href Filename = ${itemHrefFilename}`); // Added for debugging

                   // Add active class if the item's href filename matches the current page filename
                   if (itemHrefFilename === currentPageFilename) {
                       item.classList.add('active');
                       console.log(`[Debug] setActiveNavItem: Adding 'active' class to item with href=${itemHrefFilename}`); // Added for debugging
                       // console.log(`setActiveNavItem: Activating nav item for: ${itemHrefFilename}`); // Optional Debug
                   }
               }
           }
       });
            ...
            if (currencySelect) {
                ...
                this.updateBudgetCurrencySymbol(savedCurrency);
                 currencySelect.addEventListener('change', function() {
                    ...
                    // No toast here, saved on clicking "Save Settings"
                });
            }
    ...
    static updateUICurrency(currencyCode) {
        ...
        // Update placeholders maybe?
         document.querySelectorAll('input[placeholder*="$"]').forEach(input => {
             input.placeholder = input.placeholder.replace(/\$\d*|\$\s*/, `${symbol}`); // Basic replacement
         });
        ...
        if (!document.querySelector('.bottom-navigation')) {
            ...
            navElement.innerHTML = `
                <div class="max-w-lg mx-auto flex justify-around items-center w-full px-2">
                    ...
                    <button id="settingsBtn" class="nav-item flex flex-col items-center px-3 py-1 text-gray-500 dark:text-gray-400 hover:text-accent-purple dark:hover:text-accent-purple transition-colors duration-200 rounded-lg" aria-label="Open Settings">
 ...
 document.addEventListener('DOMContentLoaded', () => {
     ...
     // Ensure bottom nav exists, create if necessary (handles dynamic loading issues)
     // Use a small timeout to allow other scripts potentially modifying the DOM to run first
     setTimeout(() => {
         if (!document.querySelector('.bottom-navigation')) {
             console.log("Bottom nav check failed after DOMContentLoaded + timeout, attempting creation.");
             UI.createBottomNavIfMissing();
         } else {
             // console.log("Bottom nav confirmed present after DOMContentLoaded + timeout.");
             // Ensure active state is correct even after potential delays/updates
              UI.setActiveNavItem();
         }
         // Initialize dropdowns after potential nav creation and other UI setups
         UI.initDropdownSupport();
         // Set initial currency display based on saved preference or default
         const savedCurrency = localStorage.getItem('currency') || 'SGD'; // Default to SGD
         UI.updateUICurrency(savedCurrency);

     }, 10); // Short delay (10ms)


     // Apply theme instantly based on localStorage before full UI init (reduces flicker)
     const storedTheme = localStorage.getItem('theme');
     const systemPrefersDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
